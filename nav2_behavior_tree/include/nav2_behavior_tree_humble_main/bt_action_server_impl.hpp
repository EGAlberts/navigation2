...
#include "nav2_behavior_tree_humble_main/bt_action_server.hpp"
#include "hm_nav2_util/copy_all_parameter_values.hpp"
...
bool BtActionServer<ActionT>::on_configure()
{
  ...
  hm_nav2_util::copy_all_parameter_values(node, client_node_);
  ...
  bt_ = std::make_unique<nav2_behavior_tree_humble_main::BehaviorTreeEngine>(plugin_lib_names_, client_node_);
...
void BtActionServer<ActionT>::executeCallback()
{
  ...
  nav2_behavior_tree_humble_main::BtStatus rc = bt_->run(&tree_, on_loop, is_canceling, bt_loop_duration_);
  ...
  switch (rc) {
    case nav2_behavior_tree_humble_main::BtStatus::SUCCEEDED:
      action_server_->succeeded_current(result);
      RCLCPP_INFO(logger_, "Goal succeeded");
      break;

    case nav2_behavior_tree_humble_main::BtStatus::FAILED:
      action_server_->terminate_current(result);
      RCLCPP_ERROR(
    ...
    case nav2_behavior_tree_humble_main::BtStatus::CANCELED:
