...
/**
 * @brief Parse XML string to hm_nav2_msgs::msg::WaypointStatus
 * @param key XML string
 * @return hm_nav2_msgs::msg::WaypointStatus
 */
template<>
inline hm_nav2_msgs::msg::WaypointStatus convertFromString(const StringView key)
{
  // if string starts with "json:{", try to parse it as json
  if (StartWith(key, "json:")) {
    auto new_key = key;
    new_key.remove_prefix(5);
    return convertFromJSON<hm_nav2_msgs::msg::WaypointStatus>(new_key);
  }
  ...
  if (parts.size() != 13) {
    ...
    hm_nav2_msgs::msg::WaypointStatus waypoint_status;
 ...
 */
template<>
inline std::vector<hm_nav2_msgs::msg::WaypointStatus> convertFromString(const StringView key)
{
  // if string starts with "json:{", try to parse it as json
  if (StartWith(key, "json:")) {
    auto new_key = key;
    new_key.remove_prefix(5);
    return convertFromJSON<std::vector<hm_nav2_msgs::msg::WaypointStatus>>(new_key);
  }
  ...
  if (parts.size() % 13 != 0) {
    ...
    std::vector<hm_nav2_msgs::msg::WaypointStatus> wp_status_vector;
    for (size_t i = 0; i < parts.size(); i += 13) {
      hm_nav2_msgs::msg::WaypointStatus wp_status;
