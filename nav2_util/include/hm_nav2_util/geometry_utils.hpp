...
inline int find_next_matching_goal_in_waypoint_statuses(
  const std::vector<hm_nav2_msgs::msg::WaypointStatus> & waypoint_statuses,
  const geometry_msgs::msg::PoseStamped & goal)
{
  auto itr = std::find_if(waypoint_statuses.begin(), waypoint_statuses.end(),
      [&goal](const hm_nav2_msgs::msg::WaypointStatus & status){
        return status.waypoint_pose == goal &&
               status.waypoint_status == hm_nav2_msgs::msg::WaypointStatus::PENDING;
    });
