...
class LifecycleTransitionTestNode : public hm_nav2_util::LifecycleNode
{
public:
  explicit LifecycleTransitionTestNode(rclcpp::NodeOptions options)
  : hm_nav2_util::LifecycleNode("test_node", "", options) {}

  hm_nav2_util::CallbackReturn on_configure(const rclcpp_lifecycle::State &) override
  {
    configured = true;
    return hm_nav2_util::CallbackReturn::SUCCESS;
  }
  hm_nav2_util::CallbackReturn on_activate(const rclcpp_lifecycle::State &) override
  {
    activated = true;
    return hm_nav2_util::CallbackReturn::SUCCESS;
  }
...
TEST(LifecycleNode, RclcppNodeExitsCleanly)
{
  ...
  auto node1 = std::make_shared<hm_nav2_util::LifecycleNode>("test_node", "");
...
TEST(LifecycleNode, MultipleRclcppNodesExitCleanly)
{
  ...
  auto node1 = std::make_shared<hm_nav2_util::LifecycleNode>("test_node1", "");
  auto node2 = std::make_shared<hm_nav2_util::LifecycleNode>("test_node2", "");
...
TEST(LifecycleNode, OnPreshutdownCbFires)
{
  ...
  class MyNodeType : public hm_nav2_util::LifecycleNode
  {
public:
    MyNodeType(
      const std::string & node_name)
    : hm_nav2_util::LifecycleNode(node_name) {}
...
protected:
    void on_rcl_preshutdown() override
    {
      ...
      hm_nav2_util::LifecycleNode::on_rcl_preshutdown();
    }
  };
